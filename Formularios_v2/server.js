...
app.put('/api/usuarios/:id', async (req, res) => {
    ...
    try {
        ...
        switch (rol) {
            case 'cliente':
                sql = 'UPDATE Cliente SET nombre_cli = ?, apellido_cli = ?, telefono_cli = ?, correo_cli = ?' + (contraseña ? ', contraseña_cli = ?' : '') + ' WHERE id_cliente = ?';
                values = [nombre, apellido, telefono, correo, ...(contraseña ? [await bcrypt.hash(contraseña, 10)] : []), id];
                break;
            case 'mesero':
                sql = 'UPDATE Mesero SET nombre_msr = ?, apellido_msr = ?, telefono_msr = ?, correo_msr = ?' + (contraseña ? ', contraseña_msr = ?' : '') + ' WHERE id_mesero = ?';
                values = [nombre, apellido, telefono, correo, ...(contraseña ? [await bcrypt.hash(contraseña, 10)] : []), id];
                break;
            case 'domiciliario':
                sql = 'UPDATE Domiciliario SET nombre_domi = ?, apellido_domi = ?, telefono_domi = ?, correo_domi = ?' + (contraseña ? ', contraseña_domi = ?' : '') + ' WHERE id_domiciliario = ?';
                values = [nombre, apellido, telefono, correo, ...(contraseña ? [await bcrypt.hash(contraseña, 10)] : []), id];
                break;
            case 'administrador':
                sql = 'UPDATE Administrador SET nombre_admin = ?, apellido_admin = ?, telefono_admin = ?, correo_admin = ?' + (contraseña ? ', contraseña_admin = ?' : '') + ' WHERE id_admin = ?';
                values = [nombre, apellido, telefono, correo, ...(contraseña ? [await bcrypt.hash(contraseña, 10)] : []), id];
                break;
            default:
                return res.status(400).send('Rol no válido');
