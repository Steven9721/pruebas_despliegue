...
// Función para actualizar el precio total, considerando que los ingredientes seleccionados no se multiplican por la cantidad
function updateTotalPrice(modalNumber, basePrice) {
    ...
    let totalPrice = basePrice * quantity; // Multiplicar solo el precio base por la cantidad

    // Sumar los precios de los ingredientes seleccionados sin multiplicar por la cantidad
    let checkboxes = document.querySelectorAll(`#customizationModal${modalNumber} input[type="checkbox"]`);
    let extrasTotal = 0;
    
    checkboxes.forEach(checkbox => {
        if (checkbox.checked) {
            let extraPrice = parseInt(checkbox.getAttribute('data-price'));
            if (!isNaN(extraPrice)) {
                extrasTotal += extraPrice; // Sumar precio del ingrediente extra (solo una vez)
            }
        }
    });

    // Sumar el total de los extras al precio final
    totalPrice += extrasTotal;

    // Actualizar el precio total en el contador
